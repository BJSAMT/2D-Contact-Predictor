...
pretrained_model.load_model("pretrained_model_p.json")
...
new_data = pd.read_csv("semi-metal_features.csv") 
X_new = new_data[model_feature_names].values
y_new = new_data['SBH_p'].values
...
pred_data = pd.read_csv("semi-metal_features.csv")         
X_pred = pred_data[model_feature_names].values
y_true = pred_data['SBH_p'].values
...
# save model
updated_model.save_model("updated_model_p-semi.json")
