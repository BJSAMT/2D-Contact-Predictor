...
Y_post = Combine.drop(Combine[abs(Combine['SBH_p']-Combine['Pred'])>0.5].index).iloc[:,0]
X_post = X.drop(Combine[abs(Combine['SBH_p']-Combine['Pred'])>0.5].index)
...
plt.title('SBH_p (RFECV)', fontsize=16, fontweight='bold', pad=20)
...
plt.title('SBH_p Feature Importance')
...
save_path = "pretrained_model_p.json"
...
plt.title('SBH_p')
#plt.savefig('Regression-p.png', format='png', dpi=300, bbox_inches='tight')
plt.show()
